...
# regarding the trips/rides all drivers have completed.

# Hold the hard coded information regarding each trip
drivers_stats = {
  DR0001: [
...
# Gather all information per ride, store in designated arrays and display results as needed
...
drivers_stats.each do |driver_id, rides|
  ...
  # Average the trip's rating and store for later use
  trip_ratings << '%.2f' % ((trip_rating.sum.to_f/trip_rating.length))
  # Sum all trip cost per driver and store for later use
  total_trip_cost << (trip_cost.sum)

  # Display number of rides done for driver
  puts "\nDriver #{driver_id} had a total of #{drivers_stats[driver_id].length} rides"
  # Display earnings per trip
  puts "Making a grand total of $#{trip_cost.sum} for all the awesome rides"
...
# Calculate and display driver with the highest average rating
...
# Calculate and display driver with most earnings
